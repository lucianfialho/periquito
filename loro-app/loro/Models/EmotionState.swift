...
private let logger = Logger(subsystem: "com.ruban.loro", category: "EmotionState")

@MainActor
@Observable
final class EmotionState {
    private(set) var currentEmotion: LoroEmotion = .neutral
    private(set) var scores: [LoroEmotion: Double] = [
        .happy: 0.0,
        .sad: 0.0
    ]
    ...
    func recordEmotion(_ rawEmotion: String, intensity: Double, prompt: String) {
        let emotion = LoroEmotion(rawValue: rawEmotion)
