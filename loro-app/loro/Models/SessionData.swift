...
private let logger = Logger(subsystem: "com.ruban.loro", category: "SessionData")
...
final class SessionData: Identifiable {
    ...
    private(set) var task: LoroTask = .idle
    let emotionState = EmotionState()
    var state: LoroState {
        LoroState(task: task, emotion: emotionState.currentEmotion)
    }
    ...
    func updateTask(_ newTask: LoroTask) {
        task = newTask
        lastActivity = Date()
    }
