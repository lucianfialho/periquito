#!/bin/bash
# Periquito Hook - forwards Claude Code events to Periquito app via Unix socket

SOCKET_PATH="/tmp/periquito.sock"

# Exit silently if socket doesn't exist (app not running)
[ -S "$SOCKET_PATH" ] || exit 0

# Skip if this is Periquito's own analysis process
[ "$PERIQUITO_ANALYSIS_RUNNING" = "1" ] && exit 0
...
export PERIQUITO_INTERACTIVE=$IS_INTERACTIVE
...
output = {
    ...
    'pid': None,
    'tty': None,
    'interactive': os.environ.get('PERIQUITO_INTERACTIVE', 'true') == 'true',
